...
public class ReportsGui {
    ...
    private void open(Player staff, int requestedPage) {
        // Fetch reports async, then build and open GUI on main thread
                            ...
                            switch (info.ch) {
                                    ...
                                    return gui.createItem(Material.getMaterial(reportGui.getString("R.item", "PAPER")))
                                            ...
                                            .action((event, container) -> {
                                                if (event.isRightClick()) {
                                                    // Teleport must happen on main thread - already on it from click event
                                                    ...
                                                    plugin.getLang().get("report.marked_as_handled")
                                                            .with("%report_id%", String.valueOf(report.getId())).sendTo(staff);
                                ...
                                case 'C' -> {
                                    return gui.createItem(Material.getMaterial(reportGui.getString("C.item", "BARRIER")))
                                            .name(Text.of(reportGui.getString("C.name", "")).buildString())
                                            .lore(Text.of(reportGui.getString("C.lore", "")))
                                ...
                                case 'B' -> {
                                    return gui.createItem(Material.getMaterial(reportGui.getString("B.item", "ARROW")))
                                            .name(Text.of(reportGui.getString("B.name", "")).buildString())
                                            .lore(Text.of(reportGui.getString("B.lore", "")))
                                            .action((event, container) -> {
                                                if (currentPage > 0) {
                                ...
                                case 'N' -> {
                                    ...
                                    return gui.createItem(Material.getMaterial(reportGui.getString("N.item", "ARROW")))
                                            .name(Text.of(reportGui.getString("N.name", "")).buildString())
                                            .lore(Text.of(reportGui.getString("N.lore", "")))
                                            .action((event, container) -> {
                                                if (currentPage < totalPages - 1) {
                                                    open(staff, currentPage + 1);
                                                }
